...
#ifndef FFTN_H_98843722_85F2_11E2_8363_001485C48853
#define FFTN_H_98843722_85F2_11E2_8363_001485C48853
...
class fftn_context {
	...
	friend int fftn (int ndim, const int dims [], REAL Re [], REAL Im [], int iSign, double scaling);

	static const double PI;
	static const double SIN60; // sin(60 deg)
	static const double COS72; // cos(72 deg)
	static const double SIN72; // sin(72 deg)

	fftn_context() :
		SpaceAlloced(0), MaxPermAlloced(0),
		Tmp0(NULL), Tmp1(NULL), Tmp2(NULL), Tmp3(NULL), Perm(NULL) {
	}
	...
	void fft_free (void);
	int factorize (int nPass, int * kt);
...
const double fftn_context::PI = 3.14159265358979323846264338327950288;
const double fftn_context::SIN60 = 0.86602540378443865; // sin(60 deg)
const double fftn_context::COS72 = 0.30901699437494742; // cos(72 deg)
const double fftn_context::SIN72 = 0.95105651629515357; // sin(72 deg)

void fftn_context::fft_free (void)
{
...
int fftn_context::factorize (int nPass, int * kt)
{
	int nFactor = 0;
	int j, jj;
...
template<typename REAL>
int fftn_context::fftradix (REAL Re [], REAL Im [], size_t nTotal, size_t nPass,
	size_t nSpan, int iSign, int maxFactors, int maxPerm)
{
	...
	#endif
	REAL pi2 = PI;    // use PI first, 2 PI later

	k3 = 0; // initialize to make gcc happy

	c2 = c3 = s2 = s3 = 0.0; // initialize to make gcc happy
	...
	if (MaxPermAlloced < (unsigned int) maxPerm)
	{
		...
		if (!MaxPermAlloced) // first time
			Perm = (int*) malloc (maxPerm * sizeof(int));
		else
		#endif
			Perm = (int*) realloc (Perm, maxPerm * sizeof(int));
	...
	if (nFactor > NFACTOR)
	{
		fprintf (stderr, "Error: %s<%s>() - exceeded number of factors\n", __func__, typeid(REAL).name());
		goto Memory_Error;
	}
	...
	for (;;)
	{
		...
		switch (factor [ii - 1])
		{
			...
			#ifdef FFT_RADIX4
				fprintf (stderr, "Error: %s<%s>(): compiled for radix 2/4 only\n", , __func__, typeid(REAL).name());
	...
	Memory_Error:
	fprintf (stderr, "Error: %s<%s>() - insufficient memory.\n", __func__, typeid(REAL).name());
	fft_free (); // free-up memory
	return -1;
}
...
int fftn (int ndim, const int dims [], REAL Re [], REAL Im [], int iSign, double scaling)
{
	...
	if (dims != NULL)
	{
		...
		for (i = 0; i < ndim; i++)
		{
			int ret;
			nSpan *= dims [i];
			ret = context.fftradix<REAL> (Re, Im, nTotal, dims [i],
				nSpan, iSign, maxFactors, maxPerm);
	...
	{
		int ret;
		ret = context.fftradix<REAL> (Re, Im, nTotal, nTotal, nTotal,
			 iSign, maxFactors, maxPerm);
		// exit, clean-up already done
		if (ret)
			return ret;
	}
	...
	Dimension_Error:
	fprintf (stderr, "Error: %s<%s>() - dimension error\n", __func__, typeid(REAL).name());
	context.fft_free (); // free-up memory
	return -1;
}
...
#endif // FFTN_H_98843722_85F2_11E2_8363_001485C48853
